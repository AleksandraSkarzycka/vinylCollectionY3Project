<script lang="ts">
    ...
    function openModal(wishlist: { vinylImg: any; vinylTitle: any; vinylDesc: any; spotifyEmbed: any; } | null) {
        console.log('Clicked:', wishlist); // debug
        selectedVinyl = wishlist;
        showModal = true;
    }
...
<div class="title">
    <p>Aleksandra's Wishlist</p>
</div>

<div class="grid">
  {#each data.wishlist as wishlist (wishlist.id)}
    <!-- svelte-ignore a11y_click_events_have_key_events -->
    <!-- svelte-ignore a11y_no_static_element_interactions -->
    <div class="card" on:click={() => openModal(wishlist)}>
        <div class="vImg">
            <img src={wishlist.vinylImg} alt={wishlist.vinylTitle} />
        </div>
        <div class="vTitle">
            <h2>{wishlist.vinylTitle}</h2>
        </div>
    </div>
  {/each}
</div>
